function DESIGNVIEWER_SPAN2(Design_variables)
...
PRM.fcd=TP.fcd_data(Design_variables(5));     	% Design compressive strength of concrete
PRM.fctd=TP.fctd_data(Design_variables(5));    	% Design tensile strength of concrete
PRM.k1=TP.k1_data(Design_variables(5));        	% Equivalent compression block depth coefficient
PRM.Ec=TP.Ec_data(Design_variables(5));       	% Modulus of elasticity of concrete
PRM.fyd=TP.fyd_data(Design_variables(6));     	% Yield strength of reinforcement
PRM.CC=TP.Cc_data(Design_variables(5));        	% Unit cost of concrete
PRM.CST=TP.Cs_data(Design_variables(6));       	% Unit cost of steel
%Stirrup spacing in the confinement zone
PRM.s_sar=[TP.etr_ara(Design_variables(7)) TP.etr_ara(Design_variables(9))];
%Stirrup spacing in the mid zone
PRM.s_orta=[TP.etr_ara(Design_variables(8)) TP.etr_ara(Design_variables(10))];       
...
fprintf('Concrete Grade                           = %8s  \n',TP.Csinif(Design_variables(5)))
fprintf('Steel Grade                              = %8s  \n',TP.Fsinif(Design_variables(6)))
...
fprintf('Left support moment bearing strength at      beam #%2d = %8.2f kNm\n',i,PRM.UST_MR(i,1))
fprintf('Right support moment bearing strength at     beam #%2d = %8.2f kNm\n',i,PRM.UST_MR(i,3))

fprintf('Left support tensile reinforcement ratio at  beam #%2d = %8.4f \n',i,PRM.UST_RHO(i,1))
fprintf('Right support tensile reinforcement ratio at beam #%2d = %8.4f \n',i,PRM.UST_RHO(i,3))
fprintf('------------------------------------------------------------------------\n')
end
